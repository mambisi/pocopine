...
adapter. That means Firebase imports, generated `./bindings` payload types,
and facade signatures fail early through the Pocopine command instead of a
separate npm workflow.
...
[`pocopine-auth-jwt`](../guides/auth/jwt-providers.md). Then scope server
