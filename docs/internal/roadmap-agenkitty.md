...
| **M2** | **Artifacts tool** | `artifact.write/read/list/link/delete` over metadata + store traits; session store under `.agenkitty/sessions`, project store under `.agenkitty/artifacts`; content-hashed (via `pocopine-crypto`), media-typed, provenance-carrying; policy defaults session-write **Allow**, project-write **Ask**, delete **Ask**; process-output references. | Artifacts survive session reload with hash + media type intact; path traversal in names rejected; size limits enforced; a `process.run` output can be captured as an artifact and cited by id. |
| **M3** | **Unblocked integrations** | `net.download` (bounded binary → artifact store, content-hashed); session↔artifact links + promotion policy; session-close reaping of process handles; patch metadata → session checkpoint/event refs; session cleanup-hook adapters. | A download lands as a citable artifact honoring `NetPolicy` + byte caps; closing a session kills its live process handles; a `patch.apply` is discoverable from `session.events` by checkpoint ref. |
