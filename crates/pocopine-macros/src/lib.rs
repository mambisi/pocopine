...
pub fn app(input: TokenStream) -> TokenStream {
    ...
    if let Some(mode) = split_mode.as_deref() {
        ...
        if mode == "postlink" {
            ...
            return emit_postlink_split_app(&parsed, &split_base).into();
        }
        ...
        return syn::Error::new(
            proc_macro2::Span::call_site(),
            "POCOPINE_SPLIT_MODE must be `shell`, `postlink`, or `route:<index>`",
        )
        .to_compile_error()
        .into();
    }
...
fn emit_postlink_split_app(parsed: &AppMacroInput, split_base: &str) -> proc_macro2::TokenStream {
    let route_path_keys: std::collections::HashSet<String> = parsed
        .routes
        .iter()
        .map(|r| {
    ...
    let devtools_call = parsed.devtools.then(|| quote! { .with_devtools() });
    let manifest = split_manifest_json(parsed, split_base);
    let route_entries = parsed.routes.iter().enumerate().map(|(idx, route)| {
        let route_id = split_route_id(idx, &route.component);
        let ident_suffix = split_route_ident_suffix(idx, &route_id);
        let split_module_ident = format_ident!("pocopine_route_{ident_suffix}");
        let split_mount_ident = format_ident!("__pocopine_split_mount_route_{ident_suffix}");
        let preload_ident = format_ident!("__pocopine_preload_route_{ident_suffix}");
        let export_mount_ident = format_ident!("mount_pocopine_route_{ident_suffix}");
        let export_preload_ident = format_ident!("preload_pocopine_route_{ident_suffix}");
        let component = &route.component;
        let pattern = route.pattern.value();
        let param_setters = route_param_setters(&pattern);

        quote! {
            #[::pocopine::__private::wasm_split_helpers::wasm_split(
                #split_module_ident,
                wasm_split_path = ::pocopine::__private::wasm_split_helpers,
                preload(#preload_ident)
            )]
            fn #split_mount_ident(
                outlet: ::pocopine::__private::web_sys::Element,
                path: ::std::string::String,
            ) {
                __pocopine_unmount_active_route();
                let Some(document) = ::pocopine::__private::web_sys::window()
                    .and_then(|window| window.document())
                else {
                    return;
            ...
            #[::pocopine::__private::wasm_bindgen::prelude::wasm_bindgen]
            pub fn #export_preload_ident() -> ::pocopine::__private::js_sys::Promise {
                ::pocopine::__private::wasm_bindgen_futures::future_to_promise(async move {
                    #preload_ident().await;
                    ::core::result::Result::Ok(::pocopine::__private::JsValue::UNDEFINED)
                })
            }

            #[::pocopine::__private::wasm_bindgen::prelude::wasm_bindgen]
            pub fn #export_mount_ident(
                outlet: ::pocopine::__private::web_sys::Element,
                path: ::std::string::String,
            ) -> ::pocopine::__private::js_sys::Promise {
                ::pocopine::__private::wasm_bindgen_futures::future_to_promise(async move {
                    #split_mount_ident(outlet, path).await;
                    ::core::result::Result::Ok(::pocopine::__private::JsValue::UNDEFINED)
                })
            }
        }
    ...
    quote! {
        {
            ...
            fn __pocopine_unmount_active_route() {
                __POCOPINE_ROUTE_HANDLE.with(|handle| {
                    if let ::core::option::Option::Some(handle) = handle.borrow_mut().take() {
                        handle.unmount();
                    }
