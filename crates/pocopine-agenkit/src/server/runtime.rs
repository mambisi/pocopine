...
impl AgentLoop {
    ...
    pub(crate) async fn run_turn(
        ...
        // RFC-122 §4.1: an image-output model with no sink has nowhere to put
        // generated bytes — fail before the first provider call.
        super::provider::ensure_image_output_capturable(&model, self.inner.artifacts.is_some())?;
        ...
        let outcome: AgenkitResult<StopReason> = async {
            for _ in 0..self.config.max_steps_per_turn.max(1) {
                ...
                // RFC-122 §4.1 capture rule: inline model media goes through
                // the sink (or fails loudly) BEFORE anything persists/streams;
                // the message keeps only the url-form ref.
                super::artifact::capture_response_media(
...
mod tests {
    ...
    /// A provider whose response carries inline image bytes (the §4.1
    /// backstop path) alongside text.
    struct ImageModel;
    impl crate::server::Provider for ImageModel {
        fn id(&self) -> &str {
            "local"
        }
        fn generate<'a>(
    ...
    #[tokio::test]
    async fn model_media_is_captured_through_the_sink_and_persisted_as_a_ref() {
        let sink = MemoryArtifactSink::new();
        let agenkit = Agenkit::builder()
            .provider(ImageModel)
            .default_model(ModelRef::new("local/default"))
        ...
        let (tx, mut rx) = unbounded_channel();
        let mut messages = vec![Message::user("draw me")];
        loop_
            .run_turn(&mut messages, &tx, &TurnControls::default())
            .await
            .unwrap();

        // The sink holds the DECODED bytes; the transcript holds only a ref.
        assert_eq!(sink.len(), 1);
        ...
        let media = assistant
            ...
            .iter()
            .find_map(|p| match p {
                pocopine_agenkit_core::ContentPart::Media(m) => Some(m),
                _ => None,
            })
            .expect("assistant message keeps a media part");
        assert!(media.data_base64.is_none(), "bytes must never persist");
    ...
    #[tokio::test]
    async fn model_media_without_a_sink_fails_the_turn_loudly() {
        let agenkit = Agenkit::builder()
            .provider(ImageModel)
            .default_model(ModelRef::new("local/default"))
            .build()
        ...
        let err = loop_
            ...
            .await
            .unwrap_err();
        assert!(err.to_string().contains("no artifact sink"), "{err}");
    }

    #[tokio::test]
    async fn persisted_artifact_refs_replay_as_text_placeholders() {
        use crate::server::{GenerateRequest, GenerateResponse, Provider, ProviderContext};
        use std::sync::Mutex;
        ...
        let (tx, _rx) = unbounded_channel();
        // History as a resumed thread would load it: a prior turn's captured
        // image, persisted as a url-form media part (RFC-122 §4.1).
        let mut messages = vec![
            Message::user("draw a cat"),
            Message::assistant(Content::from_parts(vec![
                pocopine_agenkit_core::ContentPart::text("here: "),
                pocopine_agenkit_core::ContentPart::Media(pocopine_agenkit_core::MediaPart {
                    media_type: "image/png".to_string(),
                    url: Some("ak:file/cat123".to_string()),
                    data_base64: None,
        ...
        let request = last.lock().unwrap().take().expect("provider was called");
        let assistant_text = request.messages[1].text();
        assert!(
            assistant_text.contains("[generated image: cat.png (ak:file/cat123)]"),
            "{assistant_text}"
        );
        // The DURABLE history still holds the media part — only the wire view
        // was placeholdered.
        assert!(matches!(
            messages[1].content.parts[1],
            pocopine_agenkit_core::ContentPart::Media(_)
        ));
    }
