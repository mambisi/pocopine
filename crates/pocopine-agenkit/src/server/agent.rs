...
async fn run_loop<A: AiAgent>(
    ...
    super::provider::ensure_image_output_capturable(model, run.inner.artifacts.is_some())?;
    ...
    for _ in 0..config.max_steps {
        ...
        let mut response =
            loop_core::run_model_step(provider, request, &cx, model, &observer).await?;
        // RFC-122 §4.1 capture rule (typed run): inline model media goes
        // through the sink (or fails loudly) before the content persists in
        // the loop transcript.
