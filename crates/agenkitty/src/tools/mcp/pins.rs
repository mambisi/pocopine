...
impl PinStore {
    ...
    /// Clear approval for every tool of `server` whose name is **not** in
    ...
    /// Returns the tool names whose approval was revoked.
    pub fn revoke_missing(&self, server: &str, present: &HashSet<String>) -> Vec<String> {
        let mut records = self.records.lock().expect("mcp pin store poisoned");
        let mut cleared = Vec::new();
        for ((s, t), record) in records.iter_mut() {
            if s == server && record.approved && !present.contains(t) {
                record.approved = false;
