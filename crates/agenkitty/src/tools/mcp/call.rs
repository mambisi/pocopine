        ...
        match decision {
            ...
            AdmissionDecision::Allow | AdmissionDecision::Ask(_) => {
                    ...
                    .is_approved(&input.server, &tool.name, &tool.pin_hash())
                {
                    ...
                    // approval payload; resolve it through the shared approval
                    // sequence (the one copy on the runtime — see
                    // `resolve_ask_approval`). An `Allow` tool in this branch is
                    // a rug-pull (its pin was invalidated); that stays fail
                    // closed, never an approval.
                    let approved = match &decision {
                        AdmissionDecision::Ask(prompt) => {
                            self.runtime
