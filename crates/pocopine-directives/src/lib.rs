...
pub const DIRECTIVES: &[DirectiveSpec] = &[
    ...
    d("show", None, ArgReq::Forbidden, Host::Any,
      "**pp-show** — toggles `display:none` vs the element's default. Element stays in the DOM. Use `pp-if` + `pp-transition` for enter/leave.", None),
    d("text", None, ArgReq::Forbidden, Host::Any,
      "**pp-text** — sets `textContent` to the stringified expression. Prefer inline `{{expr}}` when mixing static and dynamic text.", Some("rfc-025-text-interpolation")),
    d("html", None, ArgReq::Forbidden, Host::Any,
      "**pp-html** — sets `innerHTML` to the expression value. Use with caution (XSS surface).", None),
    d("bind", Some(':'), ArgReq::Required, Host::Any,
      "**pp-bind:attr** (shorthand `:attr`) — reactive attribute binding. E.g. `:class=\"variant\"`, `:disabled=\"busy\"`.", Some("rfc-020-shorthand-prefixes")),
    d("on", Some('@'), ArgReq::Required, Host::Any,
      "**pp-on:event** (shorthand `@event`) — event handler. Modifiers: `.prevent` `.stop` `.self` `.once` `.window` `.document` `.capture` `.outside` `.debounce[.ms]`; key modifiers on keyboard events.", Some("rfc-013-pp-on-key-modifiers")),
    d("model", None, ArgReq::Optional, Host::Any,
      "**pp-model** — two-way binding. Native inputs: `pp-model=\"field\"` (`.number`/`.trim`/`.lazy`). Components: `pp-model:<prop>=\"field\"`.", Some("rfc-009-pp-model-components")),
    d("ref", None, ArgReq::Forbidden, Host::Any,
      "**pp-ref** — stores the element in component refs. Access via `refs::get::<T>(\"name\")` from Rust.", None),
    d("route", None, ArgReq::Forbidden, Host::Any,
      "**pp-route** — marks `<a>` for SPA router interception (prevents default navigation, updates the outlet).", Some("rfc-003-router")),
    d("teleport", None, ArgReq::Forbidden, Host::TemplateOnly,
      "**pp-teleport** — portal rendering. Host: `<template>` only. Body inserted at a CSS-selector target (resolved once).", Some("rfc-006-pp-teleport")),
    d("transition", None, ArgReq::Optional, Host::Any,
      "**pp-transition** — enter/leave animations. Preset `pp-transition=\"fade\"`, asymmetric `:in`/`:out`, or explicit class phases `:enter` `:enter-start` `:enter-end` `:leave`…", Some("rfc-038-animation")),
    d("anchor", None, ArgReq::Required, Host::Any,
      "**pp-anchor:placement** — floating-element positioning. Placements `top/bottom/left/right` × `start/center/end`; modifiers `.offset.N` `.flip` `.same-width`.", Some("rfc-015-pp-anchor")),
    d("roving", None, ArgReq::Optional, Host::Any,
      "**pp-roving** — roving-tabindex keyboard navigation. `.vertical` (default) `.horizontal` `.both` `.nowrap`; `.virtual` for aria-activedescendant combobox.", Some("rfc-034-pp-roving-activedescendant")),
    d("resize", None, ArgReq::Forbidden, Host::Any,
      "**pp-resize** — ResizeObserver integration. Handler gets `(width: f64, height: f64)`. Modifiers `.content-box` `.border-box` `.document`.", Some("rfc-016-pp-resize-pp-intersect")),
    d("intersect", None, ArgReq::Optional, Host::Any,
      "**pp-intersect** — IntersectionObserver integration. `:enter`/`:leave` fire on visibility; `.threshold.N` `.margin.<value>`.", Some("rfc-016-pp-resize-pp-intersect")),
    d("flip", None, ArgReq::Forbidden, Host::Any,
      "**pp-flip** — FLIP layout animation: surrounding DOM mutations animate this element from its old to new position. Honours `prefers-reduced-motion`.", Some("rfc-039-animation-v2")),
    d("as", None, ArgReq::Forbidden, Host::ComponentOnly,
      "**pp-as** — polymorphic rendering: hoists the user's child as the component's rendered root. Requires a single `<slot>` root in the component template.", Some("rfc-019-pp-as")),
    d("key", None, ArgReq::Forbidden, Host::Any,
      "**pp-key** — keyed-diffing identifier for `pp-for` (typically `item.id`). _RFC-063 (deferred) converges this to `:key` on the row root._", Some("rfc-007-pp-for-keys")),
    d("slot", None, ArgReq::Forbidden, Host::TemplateOnly,
      "**pp-slot** — consumer side of a named slot; value is the slot name. Pair with `pp-let` for scoped slots. _RFC-063 (deferred) converges to `pp-slot:name=\"binding\"`._", Some("rfc-011-scoped-slots")),
    d("let", None, ArgReq::Forbidden, Host::TemplateOnly,
      "**pp-let** — scope variable for a scoped slot's props, e.g. `<template pp-slot=\"item\" pp-let=\"ctx\">`. _RFC-063 (deferred) folds this into `pp-slot:name`._", Some("rfc-011-scoped-slots")),
    d("stagger", None, ArgReq::Forbidden, Host::TemplateOnly,
      "**pp-stagger** — per-item stagger delay (ms) for `pp-for` enter animations. _RFC-063 (deferred) converges this into a `pp-for` modifier._", Some("rfc-039-animation-v2")),
];
