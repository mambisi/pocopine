...
pub const DIRECTIVES: &[DirectiveSpec] = &[
    ...
    d(
        ...
        ArgReq::Forbidden,
        Host::Any,
        "**pp-key** — keyed-diffing identifier for `pp-for` (typically `item.id`), or instance identity on an ordinary component tag or `<pp-component>`. Changing a component key starts fresh state with current props before setup. On `<pp-component>`, `keep-alive` caches by type and key.",
