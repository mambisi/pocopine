...
pub trait SyncStreamSource: Send + Sync + 'static {
    ...
    /// `/open` (and re-sent on every `/pull` and `/push`).
    ///
    /// The default implementation accepts any params shape — sources
    /// without a declared param schema treat all subscriptions as the
    /// unparameterized base stream. Sources generated by
    /// `#[resource(params(...))]` override this with a strongly-typed
    /// validator that:
    ///
    ...
    /// open handler does not proceed to register the subscription.
