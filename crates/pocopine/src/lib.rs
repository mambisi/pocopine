...
pub use pocopine_macros::{app, component, handlers, job, protected, server, store, Emit, Props};

/// Reusable public prop group contract generated by `#[derive(Props)]`.
///
/// Component fields annotated `#[prop(flatten)]` delegate their public prop
/// leaves through this trait. In a props group, `#[prop]` fields are public
/// leaves and `#[state]` fields are explicit internal view-model state.
pub trait Props {
    /// Public prop keys exposed by this prop group.
    const KEYS: &'static [&'static str];

    /// Read one public prop leaf.
    fn get_prop(&self, key: &str) -> wasm_bindgen::JsValue;

    /// Write one public prop leaf. Unknown keys are ignored.
    fn set_prop(&mut self, key: &str, value: wasm_bindgen::JsValue);
}
...
pub mod prelude {
    ...
    pub use crate::{
        ...
        Handle, Inject, JobError, JobResult, NearestParent, Parent, Permission, Principal, Props,
        Role, RwSignal, Scope, ScopeId, ServerError, ServerResult, Setter, Signal, Store,
    };
...
pub mod __private {
    ...
    pub const fn props_key_eq(a: &str, b: &str) -> bool {
        let a = a.as_bytes();
        let b = b.as_bytes();
        if a.len() != b.len() {
            return false;
    ...
    pub const fn props_contains(keys: &[&str], key: &str) -> bool {
        let mut i = 0;
        while i < keys.len() {
            if props_key_eq(keys[i], key) {
                return true;
            }
            i += 1;
        }
    ...
    pub const fn assert_props_key(keys: &[&str], key: &str) {
        if !props_contains(keys, key) {
            panic!("flattened prop key is not declared by the Props type");
        }
    }

    pub const fn props_disjoint(left: &[&str], right: &[&str]) -> bool {
        let mut i = 0;
        while i < left.len() {
            if props_contains(right, left[i]) {
                return false;
            }
            i += 1;
        }
    ...
    pub const fn assert_props_disjoint(left: &[&str], right: &[&str]) {
        if !props_disjoint(left, right) {
            panic!("duplicate prop key after flattening");
        }
    }

    pub fn props_contains_runtime(keys: &[&str], key: &str) -> bool {
        keys.contains(&key)
    }
}
