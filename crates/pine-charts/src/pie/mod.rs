...
impl PinePieChart {
    ...
    fn on_geometry_change(
        ...
        margin_bottom: f64,
        margin_left: f64,
        inner_radius: Change<f64>,
        start_angle: Change<f64>,
        end_angle: Change<f64>,
        animate: bool,
        ...
        (
            ...
            Self::AnimatingSlices,
            Self::SliceAnimations,
    ...
    > {
        ...
        let data_changed = data.changed();
        let shape_changed = inner_radius.changed() || start_angle.changed() || end_angle.changed();
        next.data = data.current;
        next.width = width;
        next.height = height;
        ...
        next.inner_radius = inner_radius.current;
        next.start_angle = start_angle.current;
        next.end_angle = end_angle.current;
        next.animate = animate;
        next.animation_duration = animation_duration;
        next.animation_easing = animation_easing;
        crate::events::with_chart_event_target(|| {
            if data_changed {
                next.recompute_with_leaving();
            } else if shape_changed {
                next.recompute_shape();
            } else {
                next.recompute();
            }
        });
        Update::new()
            ...
            .animating_slices(next.animating_slices)
            .slice_animations(next.slice_animations)
    ...
    > {
        ...
        Update::new()
            .hover_slice(Default::default())
            .hover_overlay_visible(Default::default())
