...
impl PineAreaChart {
    ...
    fn on_geometry_change(
        points: Change<Vec<ChartPoint>>,
        series: Change<Vec<ChartAreaSeries>>,
        width: f64,
        height: f64,
        margin_top: f64,
        margin_right: f64,
    ...
    > {
        ...
        let data_changed = points.changed() || series.changed();
        next.points = points.current;
        next.series = series.current;
        next.width = width;
        next.height = height;
        ...
        Update::new()
            .exit_generation(next.exit_generation)
            .state(next.state)
            .view_box(next.view_box)
            .area_series(next.area_series)
