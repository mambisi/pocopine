...
| **Created** | 2026-05-26 |
| **Related** | [`rfc-071-event-spine-and-live-invalidation.md`](./rfc-071-event-spine-and-live-invalidation.md), [`rfc-072-offline-sync-protocol.md`](./rfc-072-offline-sync-protocol.md), [Axis 2 in `docs/sync-local-first-gaps.md`](../docs/sync-local-first-gaps.md) |
| **Supersedes** | - |
...
Drafted alongside a 4-batch implementation roadmap (see `/home/zempare-mambisi/.claude/plans/lazy-dreaming-pizza.md`):

- **Batch 1** — Wire envelope + `SyncStreamSource::validate_params`; backwards-compat regression. ~400 LOC. PR: _pending_.
- **Batch 2** — Macro `params(...)` DSL; typed `StreamParams`; subscription registry with refcount. ~900 LOC. PR: _pending_.
- **Batch 3** — Query DSL; type-safe `field::*` tokens; cache-key sharing across `stream()` and `query()`. ~500 LOC. PR: _pending_.
- **Batch 4** — Live wake-up routing with affected-fields projection; cookbook page; flip this RFC to Final. ~350 LOC + docs. PR: _pending_.

Each batch lands as a focused PR with codex review on the cumulative diff (mirroring the Axis 1 cadence).
