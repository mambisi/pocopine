...
**Implementation:** Contract, focused tests, examples, and their Pine, chart,
layout, and icon dependencies are implemented on `feat/watch-and-update`.
Downstream applications require adoption of the breaking API. This status
does not imply a merged or released feature.
...
3. **Example dependencies — migrated locally.** Pine, chart, layout, and icon
   libraries declare grouped inputs and restricted patches. Chart parts send
   configuration through named owning actions; geometry observers preserve
   selection and animation history in detached render snapshots.
4. **Integration.** Run host and wasm checks with the workspace's platform
   selections, browser regressions, and affected Pocopine CLI example builds.
   Unfiltered workspace commands also include host-only wasm dependencies and
   the macro dev-dependency cycle documented in CI.
5. **Downstream adoption — separate.** Applications outside this repository
   must migrate before upgrading to this breaking API.

Focused contract tests do not establish that every downstream application
compiles with the new API. Library/browser and example integration coverage
must accompany the consumer migrations.
