# RFC 070 - JWT-based authentication verification
...
The verifier emits diagnostic logs through the framework's tracing
stack (RFC-069); it does not use `eprintln!` or `println!`. Failed
verifications are typed `tracing` events with the `pocopine.log`
target so operators can route them to a structured sink:
...
**Privacy.** Token text and raw claim payloads are never logged. The
`Display` and `Debug` impls on `JwtAuthError` redact tokens. The
verifier emits structured `ObservedEvent`s under the
`pocopine.metric` target for ops-relevant counters
(`auth.jwt.verifications`, `auth.jwt.rejections{kind}`,
`auth.jwt.jwks.cache_miss`, `auth.jwt.jwks.refresh_rate_limited`).

**Operator alerts to set up:**
- Spike in `KeyResolutionFailed` → JWKS rotation issue or upstream
  outage.
- Spike in `SignatureInvalid` → possible attack or configuration
  drift.
- Sustained `refresh_rate_limited` > 0 → adversarial `kid` thrash.

`AuthUser.id` is logged at debug level only and never at info or
above; this matches the existing `pocopine.log` convention for PII.
