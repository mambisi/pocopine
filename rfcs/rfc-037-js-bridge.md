...
- **`.client.ts`** — optional browser-only module that imports npm
  ...
  installation, `tsc --noEmit` type-checking, and esbuild bundling;
  the `#[client_module]` macro reads the typed adapter and emits the
  Rust facade.
...
src/
  firebase/
    mod.rs                   // #[client_module] facade declaration
    auth.rs                  // app-owned Rust auth wrapper/plugin
    bindings.rs              // serde payload types
    bindings.ts              // TypeScript payload types
    Firebase.client.ts       // typed browser SDK adapter
  components/Login.poco      // normal Pocopine UI
```

The CLI scans `src/**/*.client.ts`; the Rust facade is declared
where the adapter lives. No component macro argument is required. No
`.client.ts` files means zero JavaScript bundling cost. When files
exist, the generated bundle registers their default exports by
filename-derived names: `Firebase.client.ts` becomes `firebase`, and
...
The Rust DTO is the source of truth for this file:
...
Running the app crate's `export_bindings` test refreshes the checked-in
TypeScript DTO file.
...
export default {
  ...
  async signOut(): Promise<FirebaseUser | null> {
...
pub use bindings::FirebaseUser;
```

```rust
// src/firebase/auth.rs
use super::{bindings::FirebaseUser, client};

#[derive(Clone, Default)]
pub struct FirebaseAuth;

impl FirebaseAuth {
    pub async fn sign_in(&self) -> Result<Option<FirebaseUser>, client::Error> {
        client::required()?.sign_in().await
    }
}
```

Error cases:
- Client bundle is missing ⇒ `client::required` returns an error naming the module.
...
dev server. The macro reads the `.client.ts` bridge signatures for supported
method shapes:
...
```rust
let firebase = client::required()?;
let user = firebase.sign_in().await?;
```

The next extractor phase will broaden this beyond the small bridge shapes
...
- `pocopine-cli` — `pocopine js init/install/add`, scan
  ...
  file, run `tsc --noEmit`, drive esbuild through the managed
  package manager, and inject `/pkg/pocopine-client.js` in static mode.
- Dev-server reload path mirrors the Rust hot-reload; `.client.ts`
  changes rebuild wasm because facade signatures may change.

**PR 2 — generated facade foundation.**
- `pocopine-client-codegen` — shared module discovery,
  TypeScript facade extraction, method-shape IR, and
  runtime-entry writing.
...
- Use Oxc's TypeScript parser, not a hand-rolled parser.
- Extract only the public default-export facade: async
  JSON-returning methods and callback subscriptions.
- Let the proc macro generate typed Rust methods over the hidden
...
1. **Naming.** `.client.ts` is the stable managed-module suffix.
   ...
   but generated Rust facades need a typed boundary.
